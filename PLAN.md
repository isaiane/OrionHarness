...
| O2 | Núcleo runnable | Stack Node/TS + esqueleto (T2.0) → ledger executável (T2.1) → Initializer (T2.2) → template init.sh (T2.3) → ritual get-bearings (T2.4) | em andamento | #26 (T2.0, concluída) · #29 (T2.1, concluída) · #31 (T2.2, em review) · #32 (T2.3) · #33 (T2.4) |
...
| T2.2 | Papel Initializer no pipeline (governança/doc): adiciona a fase `initialize` (bootstrap de ambiente executável), distinta do Prime, via ADR-0007 | T2 | G2 (ADR) | em review | #31 |

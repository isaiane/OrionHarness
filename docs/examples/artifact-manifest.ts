 ...
 *
 * Os padrões IMPERATIVOS (`registre no …`) ignoram a NEGAÇÃO na MESMA CLÁUSULA — o lookbehind
 * `(?<!\b(?:não|nunca|nem)\b[^.;,\n]{0,30})` alcança a negação até ~30 chars antes do verbo, parando em
 * `.`/`;`/`,`/newline (fronteira de cláusula). Assim "Não registre no `CHANGELOG.md`", "Não se deve
 * registrar no CHANGELOG.md" e "Nunca volte a registrar…" NÃO viram falso-positivo; mas a segunda
 * cláusula de "Não edite o STATE; registre no CHANGELOG.md" (drift real) AINDA morde — a negação da 1ª
 * cláusula não a alcança (achados Codex). (Os padrões "é a fonte" já são imunes: um "não" quebra a
 * adjacência `\s*(?:=|é|:)`.) Limite heurístico: negação a >~30 chars na mesma cláusula não é alcançada.
 */
...
export const MANIFEST: ManifestEntry[] = [
  ...
  // ─── historia-L5 — CHANGELOG.md como fonte autoral de história (ADR-0025 item 3) ────────────────────
  {
    ...
    group: "plan-history",
    normativeSourceRef: true,
    note: "Estubado na T9.4b (#164): deixou de ser fonte autoral (source->pointer) — história = PRs mergeados. Texto histórico existente CONGELADO (append-only, point-in-time), sem backfill nem reescrita de prosa passada.",
  },
  {
    file: "AGENTS.md",
    rule: "historia-L5",
  ...
  },
  {
    ...
    group: "plan-history",
    normativeSourceRef: true,
    note: "Get-bearings (read-path) define STATE como ponteiro — gist operacional RETIDO (D2 executa: o leitor precisa saber antes de clicar). T9.5a (#167) reduziu a duplicação do resumo do ciclo Ship (passo 5) a gist+ponteiro (§4/ADR-0024/0025); a definição do ponteiro no get-bearings permanece. Menção residual a PLAN saiu na T9.4b (#164).",
  },
  {
    file: "MEMORY.md",
    rule: "roteamento-estado",
    ...
    slice: "T9.5a",
    group: "governance-authoritative",
    note: "T9.5a (#167): o passo 4 do índice REAFIRMAVA o roteamento por-extenso (história/status/STATE enumerados) — reduzido a PONTEIRO (gist 'roteie por camada; STATE só o ponteiro' + link §4/ADR-0024). Índice = navegação (D2 explica → pointer); a regra vive na fonte. Sem normativeSourceRef (não cita mais PLAN/CHANGELOG como fonte).",
  },
  {
    file: "README.md",
    rule: "roteamento-estado",

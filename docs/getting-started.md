...
- [ ] `feature-ledger.json` — **reinicie para `[]`** (origem local): as entradas são do Orion (exemplo)
      e não se aplicam ao seu projeto. Faça pelo **fluxo normal** (branch → PR → merge) como qualquer
      mudança — o `ledger-guard` **reconhece a transição para `[]`** como o **reset de bootstrap** e a
      permite ([ADR-0021](decisions/0021-reset-bootstrap-ledger.md)), sem violar o append-only. Isso
      **estabelece a origem local** do ledger; o **append-only**
      ([ADR-0006](decisions/0006-ledger-executavel-de-tarefas.md)) volta a valer **integralmente** a
      partir dessa base (ver [ADR-0016](decisions/0016-politica-projecao-ledger.md) § Consequências).

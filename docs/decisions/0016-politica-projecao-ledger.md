...
para esta política; o `feature-ledger.json` cobre exatamente as `type:task` do escopo (nem pré-ledger,
nem duplicatas), delta aditivo com `ledger-guard` verde; a caixa de projeção no `PULL_REQUEST_TEMPLATE`
existe. Aplicação inicial: backfill de #45/#62/#67/#74 + auto-projeção do próprio #73 no PR #81.

...
- **Limitação conhecida (portabilidade, follow-up #82):** um repo criado via "Use this template" **herda**
  ...
  - **Resolução (#82, 2026-07-24 — decisão de governança aprovada pelo owner no G2; nota append-only,
    ...
    base", junto de `CHANGELOG`/`PLAN`/`STATE`), pelo **fluxo normal** (branch → PR → merge). O
    **`ledger-guard` é bootstrap-aware** ([ADR-0021](0021-reset-bootstrap-ledger.md)): reconhece a
    transição **base não-vazia → head vazio (`[]`)** como o reset one-time e a **permite** — então o
    reset passa pela via padrão (sem commit direto na `main`, compatível com rulesets de org, e a
    validação **local** também passa). **Não é exceção ao append-only sob o invariante:** o `[]`
    **estabelece a origem local**, e o append-only rege **a partir** dela — o reset é o **marco** que o
    invariante passa a proteger. *(A 1ª tentativa — reset por commit direto pré-proteção — foi
    substituída pelo guard bootstrap-aware; ver ADR-0021 §Alternativas.)* Alternativas rejeitadas:

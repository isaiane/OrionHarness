...
if ! command -v node >/dev/null 2>&1; then
  ...
  # Exercita a guarda de referência com comandos reais: um proibido (T4) e um
  # fora da allowlist (T2) devem ser BLOQUEADOS; um seguro (allowlist) LIBERADO.
...
process.exit(!blocked.allow && !outside.allow && allowed.allow ? 0 : 1);
JS

...
| **L1** Plano | GitHub **Milestones** (épico; descrição = plano completo: objetivo + bloco de design por tarefa + como iniciar, ADR-0031) + **Issues associadas** (fonte; hierarquia/status nativos, sem `- [x] → #N`) · [`PLAN.md`](PLAN.md)/[`docs/plans/`](docs/plans/) = stub-ponteiro | Mapa de épicos (no GitHub) e detalhamento |
